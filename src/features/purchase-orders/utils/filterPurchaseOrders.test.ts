...
describe("filterPurchaseOrders", () => {
  ...
  it("filters by confirm code", () => {
    ...
    expect(
      result.every((po) => po.confirmCode === "Needs Attention")
    ).toBe(true);
  });
  ...
  it("filters by assignee", () => {
    ...
    expect(
      result.every((po) => po.assignedTo === "Mohammed Rahman")
    ).toBe(true);
  });

  it("filters purchase orders within the last 30 days", () => {
    const result = filterPurchaseOrders(mockPurchaseOrders, {
      ...defaultFilters,
      selectedDateRange: "last30",
    });
  
    expect(result.length).toBeLessThan(mockPurchaseOrders.length);
  });
});